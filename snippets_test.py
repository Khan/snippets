...
class SnippetsTestBase(unittest.TestCase):
    ...
    def assertSnippetRegex(self, regex: AnyStr | Pattern[AnyStr], body, snippet_number):
        """For snippet-page 'body', assert 'regex' matches the i-th snippet.

        This works for both user-pages and weekly-pages -- we figure out the
        boundaries of the snippets, and check whether the regex matches the
        given snippet.

        Arguments:
           regex: regular expression to match against the snippet
           body: the full html page
           snippet_number: which snippet on the page to examine.
             Index starts at 0.
        """
        self.assertRegex(self._ith_snippet(body, snippet_number), regex)
...
class PostTestCase(SnippetsTestBase):
    ...
    def testPostSnippetNormal(self):
        self.login('user@example.com')
        url = '/update_snippet'
        params = {
            'week': '02-20-2012',
        ...
        response = self.request_fetcher.post(url, data=params)
        self.assertEqual(response.status_code, 200)
        self.assertDictContainsSubset({"status": 200, "message": "ok"}, response.json)
    ...
    def testPostSnippetAsOtherPerson(self):
        ...
        self.assertDictContainsSubset({"status": 403}, response.json)
        self.assertIn('joeuser@example.com', response.text)
    ...
    def testPostSnippetNotLoggedIn(self):
        ...
        self.assertDictContainsSubset({"status": 401}, response.json)
        self.assertDictContainsSubset({"message": "not logged in"}, response.json)
    ...
    def testPostSnippetIsolation(self):
        ...
        self.assertInSnippet('>my third snippet<', response.text, 0)
        self.assertInSnippet('>updated second snippet<', response.text, 1)
        self.assertNotInSnippet('>my second snippet<', response.text, 1)
        self.assertInSnippet('>my snippet<', response.text, 2)
...
class AccessTestCase(UserTestBase):
    ...
    def testCannotEditOtherSnippets(self):
        ...
        # Raises an error if we don't get a 403 response (meaning no perm).
        response = self.request_fetcher.get(url)
        self.assertEqual(response.status_code, 403)
...
class NewUserTestCase(UserTestBase):
    ...
    def testNewUserLogin(self):
        response = self.request_fetcher.get('/')
        self.assertIn('<title>New user</title>', response.text)

    def testNewUserContinueUrl(self):
        """After verifying settings, we should go back to snippet-entry."""
        response = self.request_fetcher.get('/')
        m = re.search(r'<a href="(/settings[^"]*)">', response.text)
        ...
        self.assertIn('Snippets for user@example.com', done_response.text)
    ...
    def testNewAdminWithNoAppSettings(self):
        ...
        app_settings.key.delete()

        response = self.request_fetcher.get('/', follow_redirects=True)
        self.assertIn('<title>Application settings</title>', response.text)
    ...
    def testNewAdminContinueUrls(self):
        ...
        response = self.request_fetcher.get('/', follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        m = re.search(r'name="redirect_to" value="([^"]*)"', response.text)
        continue_url = m.group(1)

        # Now kinda-simulate clicking on the submit button.
        done_response = self.request_fetcher.post(
            '/admin/update_settings',
            data={"domains": "example.com",
                  "hostname": "snippets.example.com",
                  "redirect_to": continue_url},
            follow_redirects=True
        )
        self.assertIn('<title>User settings', done_response.text)
        self.assertIn('name="redirect_to" value="snippet_entry"',
                      done_response.text)
    ...
    def testNewUserInheritsAppDefaults(self):
        ...
        self.assertRegex(settings_response.text,
    ...
    def testNewUserInValidDomain(self):
        ...
        self.assertIn('<title>User settings', response.text)
...
class UserSettingsTestCase(UserTestBase):
    ...
    def testDefaultUserSettings(self):
        ...
        self.assertInputIsNotChecked('private', response.text, 0)
        self.assertInputIsNotChecked('is_markdown', response.text, 0)

    def testPrivateUser(self):
        self.request_fetcher.get(
            '/update_settings?u=user@example.com&private=yes')
        response = self.request_fetcher.get('/')
        self.assertInputIsChecked('private', response.text, 0)
        self.assertInputIsNotChecked('is_markdown', response.text, 0)
    ...
    def testMarkdownUser(self):
        ...
        response = self.request_fetcher.get('/')
        self.assertInputIsNotChecked('private', response.text, 0)
        self.assertInputIsChecked('is_markdown', response.text, 0)

    def testSettingsForFilledInSnippets(self):
        url = '/update_snippet?week=02-21-2011&snippet=old+snippet'
        self.request_fetcher.get(url)
        response = self.request_fetcher.get('/')
        self.assertNumSnippets(response.text, 53)
        self.assertInputIsNotChecked('private', response.text, 9)
        self.assertInputIsNotChecked('is_markdown', response.text, 9)
        self.assertInSnippet('old snippet', response.text, 52)
        self.assertInputIsNotChecked('private', response.text, 52)
        self.assertInputIsNotChecked('is_markdown', response.text, 52)
        ...
        self.assertNumSnippets(response.text, 53)
        self.assertInputIsNotChecked('private', response.text, 9)
        self.assertInputIsChecked('is_markdown', response.text, 9)
        ...
        response = self.request_fetcher.get('/')
        self.assertNumSnippets(response.text, 53)
        self.assertInputIsChecked('private', response.text, 9)
        self.assertInputIsNotChecked('is_markdown', response.text, 9)
        self.assertInSnippet('old snippet', response.text, 52)
        self.assertInputIsNotChecked('private', response.text, 52)
        self.assertInputIsNotChecked('is_markdown', response.text, 52)
    ...
    def testHiddenUser(self):
        ...
        response = self.request_fetcher.get('/')
        self.assertNumSnippets(response.text, 2)
        response = self.request_fetcher.get('/weekly?week=02-27-2012')
        self.assertNumSnippets(response.text, 1)    # "no snippet this week"
        ...
        self.assertNumSnippets(response.text, 2)
        response = self.request_fetcher.get('/weekly?week=02-27-2012')
        self.assertNumSnippets(response.text, 0)
    ...
    def testDeleteButton(self):
        ...
        response = self.request_fetcher.get('/')
        self.assertNotIn('<title>New user</title>', response.text)
...
class SetAndViewSnippetsTestCase(UserTestBase):
    ...
    def testSetAndViewInUserMode(self):
        ...
        response = self.request_fetcher.get('/')
        self.assertNumSnippets(response.text, 2)
        self.assertInSnippet('>my snippet<', response.text, 0)
    ...
    def testCannotSeeInOtherWeek(self):
        ...
        self.assertNumSnippets(response.text, 1)
        self.assertNotIn('>my snippet<', response.text)
    ...
    def testViewSnippetsForTwoUsers(self):
        ...
        self.assertNumSnippets(response.text, 2)
        self.assertInSnippet('>other snippet<', response.text, 0)
        self.assertNotIn('user@example.com', response.text)
        self.assertNotIn('>my snippet<', response.text)
        response = self.request_fetcher.get('/weekly?week=02-20-2012')
        self.assertNumSnippets(response.text, 2)
        self.assertInSnippet('other@example.com', response.text, 0)
        self.assertInSnippet('>other snippet<', response.text, 0)
        self.assertInSnippet('user@example.com', response.text, 1)
        self.assertInSnippet('>my snippet<', response.text, 1)
        ...
        self.assertNotIn('>other snippet<', response.text)
        response = self.request_fetcher.get('/weekly?week=02-20-2012')
        self.assertNumSnippets(response.text, 2)
        self.assertInSnippet('other@example.com', response.text, 0)
        self.assertInSnippet('>other snippet<', response.text, 0)
    ...
    def testViewSnippetsForTwoWeeks(self):
        ...
        response = self.request_fetcher.get('/')
        self.assertNumSnippets(response.text, 3)
        # Snippets go in reverse chronological order (i.e. newest first)
        self.assertInSnippet('>my second snippet<', response.text, 0)
        self.assertInSnippet('>my snippet<', response.text, 1)

        response = self.request_fetcher.get('/weekly?week=02-20-2012')
        self.assertNumSnippets(response.text, 1)
        self.assertInSnippet('user@example.com', response.text, 0)
        self.assertInSnippet('>my snippet<', response.text, 0)
        self.assertNotIn('>my second snippet<', response.text)
    ...
    def testViewEmptySnippetsInUserMode(self):
        ...
        self.assertNumSnippets(response.text, 3)
        self.assertInSnippet('>my snippet<', response.text, 0)
        self.assertInSnippet('>my old snippet<', response.text, 2)
    ...
    def testCategorizeSnippets(self):
        ...
        # then 1 (b 2nd).
        response = self.request_fetcher.get('/weekly?week=02-20-2012')
        self.assertNumSnippets(response.text, 4)
        self.assertInSnippet('4@example.com', response.text, 0)
        self.assertInSnippet('2@example.com', response.text, 1)
        self.assertInSnippet('3@example.com', response.text, 2)
        self.assertInSnippet('1@example.com', response.text, 3)
    ...
    def testViewSnippetAfterAUserIsDeleted(self):
        ...
        response = self.request_fetcher.get('/weekly?week=02-20-2012')
        self.assertNumSnippets(response.text, 1)
        self.assertInSnippet('2@example.com', response.text, 0)
        self.assertTrue('(unknown)' in response.text)
    ...
    def testUrlize(self):
        ...
        response = self.request_fetcher.get('/weekly?week=02-20-2012')
        self.assertNumSnippets(response.text, 1)
        self.assertSnippetRegex(
            r'>visit <a href="http://foo.com".*>http://foo.com</a><',
            response.text, 0)
...
class NosnippetGapFillingTestCase(UserTestBase):
    ...
    def testTwoSnippetsInDistantPast(self):
        ...
        response = self.request_fetcher.get('/')
        self.assertNumSnippets(response.text, 53)
        self.assertInSnippet('oldish snippet', response.text, 26)
        self.assertInSnippet('old snippet', response.text, 52)
    ...
    def testSnippetInTheFuture(self):
        ...
        self.request_fetcher.get(url)
        response = self.request_fetcher.get('/')
        self.assertNumSnippets(response.text, 54)
        self.assertInSnippet('future snippet', response.text, 0)
    ...
    def testSnippetInThePastAndFuture(self):
        ...
        self.request_fetcher.get(url)
        response = self.request_fetcher.get('/')
        self.assertNumSnippets(response.text, 105)
        self.assertInSnippet('future snippet', response.text, 0)
        self.assertInSnippet('old snippet', response.text, 104)
...
class PrivateSnippetTestCase(UserTestBase):
    ...
    def testUserCanSeeAllSnippetsFromTheirDomain(self):
        ...
        self.assertNumSnippets(response.text, 4)
        self.assertInSnippet('private@some_other_domain.com', response.text, 2)
        self.assertNotInSnippet('foreign', response.text, 2)
        # We *should* see stuff from our domain, but marked private.
        self.assertInSnippet('private@example.com', response.text, 1)
        self.assertInSnippet('snippet-tag-private', response.text, 1)
        self.assertInSnippet('no see um', response.text, 1)
        # And we should see public snippets, not marked private.
        self.assertInSnippet('public@example.com', response.text, 3)
        self.assertNotInSnippet('snippet-tag-private', response.text, 3)
        self.assertInSnippet('see me', response.text, 3)

        self.login('random@some_other_domain.com')
        response = self.request_fetcher.get('/weekly?week=02-13-2012')
        self.assertNumSnippets(response.text, 4)
        self.assertInSnippet('private@some_other_domain.com', response.text, 2)
        self.assertInSnippet('foreign', response.text, 2)
        self.assertInSnippet('snippet-tag-private', response.text, 2)
        # Now we shouldn't see stuff from example.com
        self.assertInSnippet('private@example.com', response.text, 1)
        self.assertNotInSnippet('no see um', response.text, 1)
    ...
    def testPrivacyIsPerSnippet(self):
        ...
        response = self.request_fetcher.get('/weekly?week=02-13-2012')
        self.assertNumSnippets(response.text, 4)
        self.assertInSnippet('mixed@example.com', response.text, 0)
        self.assertNotInSnippet('cautious', response.text, 0)

        response = self.request_fetcher.get('/weekly?week=02-20-2012')
        self.assertNumSnippets(response.text, 4)
        self.assertInSnippet('mixed@example.com', response.text, 0)
        self.assertInSnippet('not cautious', response.text, 0)
    ...
    def testDomainMatching(self):
        ...
        self.login('user@example.com')
        response = self.request_fetcher.get('/weekly?week=02-13-2012')
        self.assertNumSnippets(response.text, 8)
        for i in (0, 1, 2, 3):    # the 4 close@ snippets should sort first
            self.assertInSnippet('close@', response.text, i)
            self.assertNotInSnippet('whoa', response.text, i)
...
class MarkdownSnippetTestCase(UserTestBase):
    ...
    def testMarkdownRendering(self):
        response = self.request_fetcher.get('/weekly?week=02-13-2012')
        self.assertInSnippet('class="snippet-text-markdown', response.text, 0)

    def testTextRendering(self):
        response = self.request_fetcher.get('/weekly?week=02-20-2012')
        self.assertInSnippet('class="snippet-text', response.text, 0)
...
class ManageUsersTestCase(UserTestBase):
    ...
    def testSortByEmail(self):
        ...
        self.assertEqual(expected, self.get_user_list(response.text))
        self.assertNotIn('@example.com deleted', response.text)  # we didn't
    ...
    def testBadSortBy(self):
        ...
        response = self.request_fetcher.get('/admin/manage_users?sort_by=unknown')
        self.assertEqual(response.status_code, 500)

    def testDelete(self):
        response = self.request_fetcher.post(
            '/admin/manage_users',
            data={
                "action": "delete",
                "email": "has_old_snippet@example.com",
            },
            follow_redirects=True)

        expected = ['has_no_snippets', 'has_many_snippets', 'has_one_snippet']
        self.assertEqual(expected, self.get_user_list(response.text))
        self.assertIn('has_old_snippet@example.com deleted', response.text)
    ...
    def testHide(self):
        ...
        expected = ['has_no_snippets', 'has_old_snippet',
                    'has_many_snippets', 'has_one_snippet']
        self.assertEqual(expected, self.get_user_list(response.text))
        self.assertIn('has_old_snippet@example.com hidden', response.text)
        self.assertIn('value="Unhide"', response.text)
    ...
    def testPreserveSortBy(self):
        ...
        self.assertIn('<th>Last snippet</th>', response.text)
    ...
    def testUnhide(self):
        ...
        expected = ['has_no_snippets', 'has_old_snippet',
                    'has_many_snippets', 'has_one_snippet']
        self.assertEqual(expected, self.get_user_list(response.text))
        self.assertIn('has_old_snippet@example.com unhidden', response.text)
        self.assertNotIn('value="Unhide"', response.text)
...
class DisplayNameTestCase(UserTestBase):
    ...
    def testUserHasEmptyDisplayName(self):
        ...
        response = self.request_fetcher.get('/weekly?week=02-20-2012')
        self.assertNumSnippets(response.text, 1)
        self.assertInSnippet('<h3>user@example.com:</h3>', response.text, 0)
